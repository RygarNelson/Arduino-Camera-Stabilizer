...
#define CUTOFF_POINT_NORTH_SINGULARTIY 0.499999
#define CUTOFF_POINT_SOUTH_SINGULARTIY -0.499999
...
float zPreviousAngleReached = 0;
float xPreviousAngleReached = 0;
float yPreviousAngleReached = 0;
...
bool gimbalLockCase(float quatReal, float quatJ, float quatK, float quatI) {
  float testGimbalLock = quatJ * quatK + quatI * quatReal;
  if (testGimbalLock < CUTOFF_POINT_SOUTH_SINGULARTIY || testGimbalLock > CUTOFF_POINT_NORTH_SINGULARTIY ) {
    return true;
  }
  else {    
    return false;
...
void getIMUData() {
  ...
  if (myIMU.dataAvailable() == true) {
    ...
    if (gimbalLockCase(quatReal, quatJ, quatK, quatI)) {
        pwm.setPWM(0, 0, angleToPulse(zPreviousAngleReached));
        pwm.setPWM(1, 0, angleToPulse(yPreviousAngleReached));
        pwm.setPWM(2, 0, angleToPulse(xPreviousAngleReached));
        Serial.println("--------------------------------------------------------------------");
        Serial.println(zPreviousAngleReached);
        Serial.println(yPreviousAngleReached);
        Serial.println(xPreviousAngleReached);
        Serial.println("--------------------------------------------------------------------");
    }
    else {
  ...
  if (zOrientationDetected == 0 || zOrientationReference == 0) {
    ...
    if (zOrientationDetected != zOrientationReference) {
      ...
      if (checkMaxMin(angleToReach)) {
        ...
        zPreviousAngleReached = angleToReach;
  ...
  if (xOrientationDetected == 0 || xOrientationReference == 0) {
    ...
    if (xOrientationDetected != xOrientationReference) {
      ...
      if (checkMaxMin(angleToReach)) {
        ...
        digitalWrite(9, LOW);
        Serial.println(angleToReach);
        xPreviousAngleReached = angleToReach;
  ...
  if (yOrientationDetected == 0 || yOrientationReference == 0) {
    ...
    if (yOrientationDetected != yOrientationReference) {
      ...
      if (checkMaxMin(angleToReach)) {
        ...
        digitalWrite(9, LOW);
        Serial.println(angleToReach);
        yPreviousAngleReached = angleToReach;
